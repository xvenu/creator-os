"""Pulse-owned Contract V2 tracking (football). Non-blocking, retryable."""
...
class FootballZozaTracker:
    """request_id, local package ID, factory state, timestamps, retries,
    export availability, failure reason."""
    ...
    def update_state(self, request_id: str, state: str,
        ...
        rec["failure_reason"] = failure_reason if failure_reason else (
            "" if state != "failed" else rec["failure_reason"])
        return rec
