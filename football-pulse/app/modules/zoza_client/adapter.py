"""Thin Contract V2 adapter: FootballPulse decision -> factory -> export.

No rendering and no publishing in this module. Publishing stays in the
Pulse publisher; analytics/revenue/learning stay Pulse-owned.
"""
...
class FakeFactoryTransport:
    """In-memory factory double (REQUEST_STATES) for tests."""
    ...
    def submit(self, payload: dict) -> dict:
        ...
        if rid in self.requests:
            return {"ok": True, "request_id": rid,
                    "state": self.requests[rid]["state"], "reused": True}
...
class HttpFactoryTransport:
    """Production transport over the zoza-factory HTTP API."""
    ...
    def status(self, request_id: str) -> dict:
        try:
            ...
            return {"ok": True, "request_id": request_id,
                    "state": r.json().get("state", "created")}
        except Exception as exc:
            return {"ok": False, "error": str(exc)[:200], "retryable": True}
...
class FootballZozaAdapter:
    """Caller-side adapter (spec name)."""

    def __init__(self, transport: Any | None = None,
                 tracker: FootballZozaTracker | None = None,
                 max_attempts: int = MAX_ATTEMPTS) -> None:
        self.transport = transport or HttpFactoryTransport()
        self.tracker = tracker or FootballZozaTracker()
    ...
    def submit_package(self, package: dict, **overrides) -> dict:
        ...
        local_id = str(package.get("package_id", package.get("id", package.get("topic"))))
        content_key = f"{local_id}:{contract['request_id']}"
    ...
    def poll(self, request_id: str) -> dict:
        ...
        if not result.get("ok"):
            ...
            self.tracker.update_state(request_id, "failed",
                                      failure_reason=str(result.get("error", "")))
            return {"request_id": request_id, "state": "failed", "error": result.get("error")}
